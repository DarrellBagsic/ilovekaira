...
# Security Groups
resource "aws_security_group" "tier2_dev" { //change //changed
  name        = "tier2_dev" //name to be changed //changed
...
resource "aws_instance" "kpair_2" {//change
  ami                         = "ami-0ffac3e16de16665e" //change this for your  specific ami //already changed
  instance_type               = "t2.micro"
  subnet_id                   = aws_subnet.private_subnets["tier2_dev"].id //change
  security_groups             = [aws_security_group.tier2_dev.id] //change
  associate_public_ip_address = true
  //key_name                    = data.aws_key_pair.key_card //change
  key_name                    = data.aws_key_pair.kpair_2.key_name //change
  #   iam_instance_profile        = "CloudWatchAgentServerPolicy"

  tags = {
    Name = "tier2_dev" //change
  }
}
